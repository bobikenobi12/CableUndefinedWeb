...
export type PortState = "closed" | "closing" | "open" | "opening" | "ready";
...
const SerialProvider = ({
	...
	const readUntilClosed = async (port: SerialPort) => {
		if (port.readable) {
			...
			let partialMessage = ""; // Variable to store partial message
			...
			try {
				while (true) {
					...
					// Check for "Ready" message and update portState if found
					if (partialMessage.includes("Ready")) {
						partialMessage = "";
